...
def _import_ocr():
    """
    Return BankStatementOCR if the ocr_extractor module exists,
    otherwise return None.  Caller must handle the None case.
    """
    try:
        # pyrefly: ignore [missing-import]
        from ocr_extractor import BankStatementOCR  # noqa: PLC0415
        return BankStatementOCR
    except ImportError:
        return None
...
async def run_analysis(
    ...
    try:
        ...
        if file_type == "pdf":
            BankStatementOCR = _import_ocr()
            if BankStatementOCR is None:
                raise RuntimeError(
                    "BankStatementOCR is not available. "
                    "Ensure ocr_extractor.py is present and its dependencies are installed."
                )
            # Run blocking OCR in a thread so we don't block the event loop
            loop = asyncio.get_event_loop()
            df: pd.DataFrame = await loop.run_in_executor(
                None, lambda: BankStatementOCR().extract(file_path)
            )
        else:
            # CSV or image treated as CSV (images would fail gracefully here;
            # add image-to-CSV logic when image ingestion is supported)
            loop = asyncio.get_event_loop()
