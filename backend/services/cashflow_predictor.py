...
def calculate_metrics(df: pd.DataFrame) -> dict[str, Any]:
    ...
    if not daily_spend.empty:
        ...
        trimmed_sum = trimmed_daily.sum()
        days_in_period = calendar_days
        
        estimated_daily_spending = _f(trimmed_sum / days_in_period) if days_in_period > 0 else 0.0
...
def generate_forecast(metrics: dict[str, Any]) -> dict[str, Any]:
    ...
    calendar_days = metrics.get("calendar_days", 1)
    credit_rows = metrics.get("_credit_rows", pd.DataFrame())

    balance = max(balance, 0)
    base_date = end.date() if isinstance(end, pd.Timestamp) else date.today()

    monthly_income = 0.0
    if not credit_rows.empty and "date" in credit_rows.columns:
        cr = credit_rows.dropna(subset=["date"])
        cr = cr[cr["amount"] >= RECURRING_INCOME_MIN_AMOUNT]
        months = max(calendar_days / 30.44, 1.0)
        monthly_income = cr["amount"].sum() / months

    net_daily_burn = rate - (monthly_income / 30.44)
    
    if net_daily_burn > 0:
        runway_days = int(balance / net_daily_burn)
    ...
    for offset in range(0, FORECAST_HORIZON_DAYS + 1, 7):
        projected_balances.append({
            ...
            "projected_balance": round(max(current_proj, 0.0), 2),
        })
        
        current_proj -= (rate * 7)
        
        days_since_income += 7
        if days_since_income >= 30:
            current_proj += monthly_income
            days_since_income -= 30
