...
export async function uploadAndAnalyze(file, onProgress) {
  ...
  // Simulate progress ticks during the long server call
  let tick = 10
  const timer = setInterval(() => {
    tick = Math.min(tick + 8, 88)
    const stages = ['queued', 'ocr', 'scoring', 'llm', 'done']
    const stage  = stages[Math.floor(tick / 20)] ?? 'scoring'
    onProgress?.({ status: stage, progress: tick })
  }, 1800)
  ...
  try {
    ...
    if (res.status === 401) throw new Error('Please log in before uploading a statement.')
    if (!res.ok) {
      const body = await res.json().catch(() => ({}))
      throw new Error(body.detail ?? `Analysis failed (HTTP ${res.status})`)
    }

    // /api/analyze returns the exact shape Analyze.jsx expects
