...
class MLScorer:
    ...
    def __init__(self, model_dir: str = CONFIG.MODELS_DIR) -> None:
        ...
        self.rules_only = (self.xgb is None and self.rf is None and self.iso is None)
        if self.rules_only:
            logger.warning("[MLScorer] No models loaded. Running in RULES_ONLY mode.")
...
class FusionLayer:
    ...
    def _calibrate(self, raw: float) -> float:
        ...
        stretched = 100.0 / (1.0 + np.exp(-k * (raw - 50.0)))
        calibrated = self.config.CALIBRATION_STRETCH_WEIGHT * stretched + self.config.CALIBRATION_RAW_WEIGHT * raw
